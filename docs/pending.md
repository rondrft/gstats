...
leaving the cache key, the `pass` design's three disagreeing margins,
`bg=transparent` leaving four designs almost blank, and the sample cards in the
README, which are now file snapshots the suite holds to the renderer rather than
files somebody has to remember to redraw.
